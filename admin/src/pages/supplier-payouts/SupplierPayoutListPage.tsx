/**
 * 파일 경로: /src/pages/supplier-payments/SupplierPayoutListPage.tsx
 * 작성 날짜: 2025-10-14
...
import { getAllPayouts } from '../../services/supplierPayoutService';
import { PAYOUT_METHOD_LABELS, PAYOUT_STATUS_LABELS } from '../../types/supplierPayout';
// import type { SupplierPayout } from '../../types/supplierPayout';

const SupplierPayoutListPage = () => {
  ...
  const loadPayments = async () => {
    ...
    try {
      ...
      const rows = data.map((payment, index) => ({
        id: index + 1,
        payoutNumber: payment.payoutNumber,
        supplierName: payment.supplierInfo.businessName,
        payoutMethod: PAYOUT_METHOD_LABELS[payment.payoutMethod],
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        return (
          <Chip
            label={PAYOUT_STATUS_LABELS[params.value as keyof typeof PAYOUT_STATUS_LABELS] || params.value}
            color={status.color}
            size="small"
          />
        );
            ...
            <Box sx={{ height: 'calc(100vh - 200px)', width: '100%' }}>
              <DataGrid
                ...
                initialState={{
                  ...
                  sorting: { sortModel: [{ field: 'payoutDate', sort: 'desc' }] }
                }}
...
export default SupplierPayoutListPage;
