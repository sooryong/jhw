...
import { getAllCollections } from '../../services/customerCollectionService';
import { COLLECTION_METHOD_LABELS, COLLECTION_STATUS_LABELS } from '../../types/customerCollection';
  ...
  const loadPayments = async () => {
    ...
    try {
      ...
      const rows = data.map((payment, index) => ({
        id: index + 1,
        collectionNumber: payment.collectionNumber,
        customerName: payment.customerInfo.businessName,
        collectionMethod: COLLECTION_METHOD_LABELS[payment.collectionMethod],
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        return (
          <Chip
            label={COLLECTION_STATUS_LABELS[params.value as keyof typeof COLLECTION_STATUS_LABELS] || params.value}
            color={status.color}
            size="small"
          />
        );
            ...
            <Box sx={{ height: 'calc(100vh - 200px)', width: '100%' }}>
              <DataGrid
                ...
                initialState={{
                  ...
                  sorting: { sortModel: [{ field: 'collectionDate', sort: 'desc' }] }
                }}
